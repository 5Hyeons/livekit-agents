...
def is_user_logging_enabled(user_identity: str) -> bool:
    """
    Check if logging is enabled for a specific user.
    
    Args:
        user_identity: User's participant identity
    ...
    return user_identity in ALLOWED_LOGGING_USERS


def is_tts_logging_enabled(user_identity: str) -> bool:
    """
    ...
    """
    if not is_user_logging_enabled(user_identity):
        return False
    
    return LOGGING_SETTINGS["tts_logging"]["enabled"]


def is_stt_logging_enabled(user_identity: str) -> bool:
    """
...
def is_metrics_logging_enabled(user_identity: str) -> bool:
    """
...
def get_tts_output_directory(user_identity: str) -> str | None:
    """
    ...
    """
    if not is_tts_logging_enabled(user_identity):
        return None
    ...
    if LOGGING_SETTINGS["tts_logging"]["create_user_folders"]:
        ...
        safe_user_id = user_identity.replace("/", "_").replace("\\", "_")
        return os.path.join(base_dir, tts_dir, safe_user_id)
    else:
        return os.path.join(base_dir, tts_dir)


def get_stt_input_directory(user_identity: str) -> str | None:
    """
    ...
    Returns:
        str | None: Directory path if logging is enabled, None otherwise
    """
    if not is_stt_logging_enabled(user_identity):
        return None
...
def get_metrics_output_directory(user_identity: str) -> str | None:
    """
    ...
    Returns:
        str | None: Directory path if logging is enabled, None otherwise
    """
    if not is_metrics_logging_enabled(user_identity):
        return None
...
def ensure_logging_directories(user_identity: str) -> Dict[str, str | None]:
    """
    ...
    tts_dir = get_tts_output_directory(user_identity)
    if tts_dir:
        try:
    ...
    stt_dir = get_stt_input_directory(user_identity)
    if stt_dir:
        try:
    ...
    metrics_dir = get_metrics_output_directory(user_identity)
    if metrics_dir:
        try:
